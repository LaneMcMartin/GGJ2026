...
func _start_player_climbing(player: Player, allow_both_raycasts: bool = false) -> void:
	...
	if (right_collision is Player) and (left_collision is Player):
		...
		if not allow_both_raycasts:
			return 
		# When player already overlaps with a disabled ladder which we then enable, 
		# start climbing in current direction.
		player.start_climbing(player.get_current_direction(), global_position.x)
